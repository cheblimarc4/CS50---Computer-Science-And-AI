...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0]-1, cell[0]+2):
            for j in range(cell[1]-1, cell[1]+2):
                if (i, j) == (cell[0], cell[1]):
                    continue
                elif i<0 or j<0 or i>self.width-1 or j>self.height-1:
                    continue
                else:
                    neighbors.add((i,j))
        #             print(j,i)
        ...
        to_add = []
        for sentence in self.knowledge:
            if s.cells and sentence.cells != s.cells:
                if sentence.cells.issubset(s.cells):
                    to_add.append(Sentence(s.cells-sentence.cells, s.count-sentence.count))
                if s.cells.issubset(sentence.cells):
                    to_add.append(Sentence(sentence.cells-s.cells, sentence.count-s.count))
        self.knowledge += to_add
    ...
    def make_random_move(self):
        ...
        """
        all_moves = set()
        for i, r in enumerate(range(self.width)):
            for j, c in enumerate(range(self.height)):
                all_moves.add((i,j))

        moves = list(all_moves-self.moves_made-self.mines)
        if moves == []:
            return None
        else:
